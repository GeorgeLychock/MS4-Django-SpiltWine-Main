...
class ProductForm(forms.ModelForm):
    ...
    class Meta:
        model = Wine
        fields = ('name', 'vintage', 'brand', 'sku', 'featured', 'image',
                  'img_url', 'has_sizes', 'size', 'measure', 'price',
                  'description', 'country_state', 'region', 'appellation',
                  'wine_type', 'varietal', 'body', 'style', 'abv', 'taste',)
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        friendly_names_apps = [(n.id, n.get_friendly_name())
                               for n in appellations]
        friendly_names_regions = [(n.id, n.get_friendly_name())
                                  for n in regions]
        friendly_names_types = [(n.id, n.get_friendly_name())
                                for n in winetypes]
        friendly_names_countries = [(n.id, n.get_friendly_name())
                                    for n in countries]
        friendly_names_varietals = [(n.id, n.get_friendly_name())
                                    for n in varietals]
